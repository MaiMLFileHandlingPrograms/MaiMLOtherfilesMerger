...
window.onload = function getFormValues() {
    ...
    datavalue = openDataerea(data_json, datavalue);
    // end data========================================================================================================================
    ...
    eventLogvalue = openEventLogerea(eventLog_json, eventLogvalue);
    //end eventLog========================================================================================================================
};
/////////////////////////////////////////////////////////////////////////////////////////////////////////

////////////////////////////////////////////////////////////////////////////////////////////////////
// document要素のコンテンツをhtml展開
//////////////////////////////////////////////////
function openDocumentarea(document_json, documentvalue){
    const document_keylist = Object.keys(document_json)
    for (let k1 in document_keylist){
        if (Object.prototype.toString.call(document_json[document_keylist[k1]])==="[object Array]" || Object.prototype.toString.call(document_json[document_keylist[k1]])==="[object Object]"){
            const details1 = document.createElement('details') // アコーディオンエリア
            details1.setAttribute('open', '')
            details1.className = div2class
            const summary1 = document.createElement('summary')
            summary1.innerHTML = document_keylist[k1] //document直下のオブジェクトタイプの要素名
            details1.appendChild(summary1)
            //for keylist[k1].lengthぶん
            for (let i = 0; i < document_json[document_keylist[k1]].length; i++){ //Object型の場合1要素が複数存在することを前提
                //documentの孫要素用のdivタグを生成
                const docchild_json = document_json[document_keylist[k1]][i] // 子要素１つが持つコンテンツ
                const docchild_keyList = Object.keys(docchild_json)  //docchild_jsonのkeyを全て取得
                for(let k2 in docchild_keyList){
                    const div2 = document.createElement('div');
                    div2.id = document_keylist[k1] + '_id'
                    div2.className = div2class
                    if (Object.prototype.toString.call(docchild_json[docchild_keyList[k2]]) === "[object Array]" || Object.prototype.toString.call(docchild_json[docchild_keyList[k2]]) === "[object Object]") {
                        ...
                        var htmlID = docchild_keyList[k2] + docchild_json['uuid'] //uuid持たない要素もある？
                        var p1 = {}
                        if (document_keylist[k1].substring(0, 1) === "@") {
                        ...
                        p1.innerHTML = `<label class="${labelclass}">${document_keylist[k1]}:</label><input type="text" id="${htmlID}" value="${document_json[document_keyList[k1]]}" readonly />`
                        documentvalue.appendChild(p1)
                    }
        ...
        else{ //document直下のObjectタイプ以外の要素
            var htmlID = document_keylist[k1] + document_json['uuid']
            var p1 = {}
            if (document_keylist[k1].substring(0, 1) === "@") {
                p1 = document.createElement('p');
                //p1 = document.createElement('span');
...
// data要素のコンテンツをhtml展開
//////////////////////////////////////////////////
function openDataerea(data_json, datavalue){
    ...
    for (let k1 in data_keyList) {
        if (Object.prototype.toString.call(data_json[data_keyList[k1]]) === "[object Array]" || Object.prototype.toString.call(data_json[data_keyList[k1]]) === "[object Object]") {
            //console.log(data_json[data_keyList[k1]])
            const details1 = document.createElement('details')
            details1.setAttribute('open', '')
            details1.className = div2class
            const summary1 = document.createElement('summary')
            summary1.innerHTML = data_keyList[k1]  //要素名:data要素直下のObjectタイプの要素名（results）
            details1.appendChild(summary1)
            //孫をもつdivタグを生成
            const div2 = document.createElement('div');
            ...
            //results => 1 だが、システム内でresults=1が前提
            ...
            for (let k2 in results_keyList) { //resultsが持つキーの数分ループ
                if (Object.prototype.toString.call(results_json[results_keyList[k2]]) === "[object Array]" || Object.prototype.toString.call(results_json[results_keyList[k2]]) === "[object Object]") {  //１以上の要素の場合
                    ...
                    for (let k3s in instances_keyList) {  //listから一つずつ(0,1,2,3)
                        ...
                        console.log('key:::::' + results_keyList[k2])
                        //divタグを生成
                        ...
                        for (let k3 in instance_keyList) {
                            if (Object.prototype.toString.call(instance_json[instance_keyList[k3]]) === "[object Array]" || Object.prototype.toString.call(instance_json[instance_keyList[k3]]) === "[object Object]") {
                                console.log(instance_keyList[k3])
                                console.log(instance_json[instance_keyList[k3]])
...
////////////////////////////////////////////////////////////////////////////////////////////////////
// eventLog要素のコンテンツをhtml展開
//////////////////////////////////////////////////
function openEventLogerea(eventLog_json, eventLogvalue) {
    ...
    for (let k1 in eventlog_keyList) {
        if (Object.prototype.toString.call(eventLog_json[eventlog_keyList[k1]]) === "[object Array]" || Object.prototype.toString.call(eventLog_json[eventlog_keyList[k1]]) === "[object Object]") {  //１以上の要素の場合
            for (let i = 0; i < eventLog_json[eventlog_keyList[k1]].length; i++){
                const details1 = document.createElement('details')
                details1.setAttribute('open', '')
                details1.className = div2class
                const summary1 = document.createElement('summary')
                summary1.innerHTML = eventlog_keyList[k1]  //logのためのHTMLタグを生成
                details1.appendChild(summary1)
                //孫をもつdivタグを生成
                const div2 = document.createElement('div');
                ...
                //console.log('eventlog_keyList[k1]:: '+eventlog_keyList[k1]) //->log
                //const log_json = eventLog_json[eventlog_keyList[k1]][0];  //logを１つ
                const log_json = eventLog_json[eventlog_keyList[k1]][i];  //log はlist[]

                //console.log('log_jsonがArrayか？　'+ Array.isArray(log_json))
                const log_keyList = Object.keys(log_json)
                for (let k2 in log_keyList) {
                    //console.log('[log_keyList[k2]]::　' + [log_keyList[k2]])
                    if (Object.prototype.toString.call(log_json[log_keyList[k2]]) === "[object Array]" || Object.prototype.toString.call(log_json[log_keyList[k2]]) === "[object Object]") {  //１以上の要素の場合
                        ...
                        for (let i = 0; i < log_json[log_keyList[k2]].length; i++) {
                            //console.log(i)
                            //console.log(log_json[log_keyList[k2]][i])
                            const details2 = document.createElement('details')
                            details2.setAttribute('open', '')
                            details2.className = div3class
                            const summary2 = document.createElement('summary')
                            ...
                            /*
                            console.log(log_keyList[k2])
                            console.log(log_json[log_keyList[k2]])
                            console.log(log_json[log_keyList[k2]][0])
                            */

                            const trace_json = log_json[log_keyList[k2]][i];
    ...
    for (let k1 in eventlog_keyList) {
        if (Object.prototype.toString.call(eventLog_json[eventlog_keyList[k1]]) === "[object Array]" || Object.prototype.toString.call(eventLog_json[eventlog_keyList[k1]]) === "[object Object]") {  //１以上の要素の場合
            for (let i = 0; i < eventLog_json[eventlog_keyList[k1]].length; i++){
                ...
                const log_json = eventLog_json[eventlog_keyList[k1]][i];  //eventLog はlist[]を持つ
                const log_keyList = Object.keys(log_json)
                for (let k2 in log_keyList) {
                    if (Object.prototype.toString.call(log_json[log_keyList[k2]]) === "[object Array]" || Object.prototype.toString.call(log_json[log_keyList[k2]]) === "[object Object]") {  //１以上の要素の場合
                        for (let j = 0; j < log_json[log_keyList[k2]]; j++){
                            //const trace_json = log_json[log_keyList[k2]][0];
                            const trace_json = log_json[log_keyList[k2]][j];
                            const trace_keyList = Object.keys(trace_json)
                            for (let k3 in trace_keyList) {
                                if (Object.prototype.toString.call(trace_json[trace_keyList[k3]]) === "[object Array]" || Object.prototype.toString.call(trace_json[trace_keyList[k3]]) === "[object Object]") {  //１以上の要素の場合
                                    //event_jsonの数分ループ
